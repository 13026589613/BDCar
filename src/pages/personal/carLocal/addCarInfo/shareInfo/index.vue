<template>
  <div class="step-container">
    ...
    <div style="width: 100%;">
      <van-cell title="共享类型">
        <div>
          <van-tag :plain="reuseType !== 0" 
            :color="reuseType === 0 ? '#F5D039' : '#999999'" 
            @click="chooseReuseType(0)">
            <span class="c-black">周期</span>
          </van-tag>
          <van-tag :plain="reuseType !== 1" class="mar-lft-5" 
            :color="reuseType === 1 ? '#F5D039' : '#999999'" 
      ...
      <van-cell title="重复时段">
        <div>
          <van-tag :plain="schemeType !== 0" 
            :color="schemeType === 0 ? '#F5D039' : '#999999'" 
            text-color="#333333" @click="chooseSchemeType(0)">

            <span class="c-black">工作日</span>
          </van-tag>
          <van-tag :plain="schemeType !== 1" class="mar-lft-5" 
            :color="schemeType === 1 ? '#F5D039' : '#999999'" 
            @click="chooseSchemeType(1)">

            <span class="c-black">双休日</span>
          </van-tag>
          <van-tag :plain="schemeType !== 2" class="mar-lft-5" 
            :color="schemeType === 2 ? '#F5D039' : '#999999'" 
            @click="chooseTime(2)">
      ...
      <van-cell title="自定义时段" is-link v-if="schemeType == 2">
        <van-tag class="mar-lft-5" 
          v-for="(item, index) in resultInfo" :key="index"
            color="#F5D039">
      ...
      <van-cell title="结束时间" 
        :value="(endTime !== null ? (overDay === 1 ? '次日 ' : '') : '') + (endTime || '选择结束时间点')" 
        is-link value-class="c-gray" @click="chooseEndTime()" />
    </div>

    <div class="mar-top" style="width: 100%;">
      <van-field :value="price" center clearable label="共享单价" @change="priceChange"
        placeholder="如设置3.5元/时，输入3.5即可" :border="false" input-align="right">
      </van-field>
    </div>
    ...
    <van-popup :show="showPopWeek" position="bottom" @clickOverlay="onCancel">
      <div @click="onCancelweek()" 
        style="padding: 1em; border-bottom: 1px solid #f5f5f5; float: left;">取消</div>

      <div @click="onCancel()" 
        style="padding: 1em; border-bottom: 1px solid #f5f5f5; float: right;">确定选择</div>
      <div style="clear: both;"></div>
      <van-checkbox-group :value="result" @change="onChange" style="padding: 1em 0;">
        <van-cell-group >
          <van-cell
            v-for="(cellItem, cellIndex) in list"
            :key="cellIndex"
            :title="cellItem"
            :data-name="cellIndex"
            @click="toggle"
            clickable
          >
            <van-checkbox :name="cellIndex" @click="checkboxClick"
              :class="'checkboxes-' + cellIndex"
...
<script>
import { operateSechmeInfo } from '../../../../../api/parking.js'
export default {
  data() {
    return {
      canJump: true,
