<template>
  <div class="full-con">
    ...
    <div class="full-container">
      <van-tabs :active="activeTab" @change="onChange" color="#E0BA21" :class="{'bg-white': activeTab === 0}">
        <van-tab title="扫码泊车"></van-tab>
        <van-tab title="预约车位"></van-tab>
      </van-tabs>

      <!-- tab内容 -->
      <scan-index v-if="activeTab === 0" @turn-to-login="turnToLogin" :showLogin="showLogin"></scan-index>
      <order-index v-if="activeTab === 1" ref="orderIndex" :showLogin="showLogin" :showHasUsingOrder="showHasUsingOrder"
        :hasUsingOrder="hasUsingOrder" :location="location" @turn-personal="turnToPersonal" @turn-to-login="turnToLogin"
        @change-type="changeType" @tip-marker="tipMarker"></order-index>
    </div>
  </div>
</template>
<script>
