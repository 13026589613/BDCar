    ...
    <map id="map" 
      ...
      <div class="parking-container">
        ...
        <div class="parking-content">
          ...
          <p class="parking-info">停车时间、收费标准以共享车位出租户设置时间为准。</p>
        </div>
      </div>
    </map>
    
    <van-tabs color="#E0BA21" class="bg-white" @change="onChange"
      style="border-bottom: 1px #f5f5f5 solid; margin-top: .3em"
      v-if="!scanInfo">
      <van-tab v-for="(enterItem, index) in enterList" :key="index"
        :title="enterItem.enterName + '进入'"></van-tab>
    </van-tabs>

    <div class="car-position-container">
      <van-row gutter="8">
        <form @submit="chooseListItem" :report-submit="true" style="width: 0" 
          v-for="(item, index) in stationList" :key="index" :id="item.userLockId">
          <van-col span="12">
...
import { loadParkEnterList, loadParkEnterStationList, saveUserForm } from '../../api/parking.js'
export default {
  // onUnload: function () {
  //   wx.redirectTo({
  //     url: '/pages/home/main'
