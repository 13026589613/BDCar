<template>
  ...
  <div class="record-container">
    ...
    <div class="panel pad-all car-place-container mar-top" 
       v-if="totalNumber > 0">
...
<script>
import { record } from '../../../api/car.js'
import { orderList } from '../../../api/parking.js'
export default {
  data() {
    return {
      record: {
  ...
  methods: {
    ...
    turnPage(url, params) { // 跳转界面
      wx.navigateTo({ url: url + '?orderId=' + params.id })
    },
    initOrderList() { // 加载列表
      const _this = this
      orderList({
        ...
        setTimeout(() => {
          if (typeof(_this.turnOrderId) !== 'undefined' && _this.turnOrderId !== 'undefined' 
            ...
            wx.navigateTo({ url: '../record/details/main' + '?orderId=' + _this.turnOrderId })
          }
        }, 300)
      })
    },
